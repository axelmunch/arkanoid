...
bool rect_rect_collision(Rectangle rect1, Rectangle rect2) {
    return (rect1.origin.x <= rect2.origin.x + rect2.height &&
            rect1.origin.x + rect1.height > +rect2.origin.x &&
            rect1.origin.y <= rect2.origin.y + rect2.width &&
            rect1.origin.y + rect1.width >= rect2.origin.y);
}
